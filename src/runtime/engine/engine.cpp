...
public:
    ...
    using Core59      = runtime::EngineCore<targets::Qwen3_5_9BInstance>;
    ...
    using ScoreCore59 = runtime::CausalScoreCore<targets::Qwen3_5_9BInstance>;
    using Core = std::variant<std::monostate, std::unique_ptr<Core27>, std::unique_ptr<Core35>,
                              std::unique_ptr<Core59>, std::unique_ptr<ScoreCore27>,
                              std::unique_ptr<ScoreCore35>, std::unique_ptr<ScoreCore59>>;
        ...
        core = std::visit(
            [&](auto& target_ptr) -> Core {
                ...
                if constexpr (std::is_same_v<Instance, targets::Qwen3_6_27BInstance>) {
                    ...
                    static_assert(std::is_same_v<Instance, targets::Qwen3_5_9BInstance>);
                    if (options.purpose == EnginePurpose::CausalScoring) {
                        return std::make_unique<ScoreCore59>(*target_ptr, device);
                    }
                    return std::make_unique<Core59>(*target_ptr, device, options,
                                                    std::move(constructed.context_cost));
                }
            },
            active);
        ...
        [&](auto& core) -> GenerationHandle {
            ...
            if constexpr (std::is_same_v<CoreState, std::monostate>) {
                throw std::logic_error("Engine core is unavailable");
                                 ...
                                 std::is_same_v<CoreState, std::unique_ptr<Impl::ScoreCore35>> ||
                                 std::is_same_v<CoreState, std::unique_ptr<Impl::ScoreCore59>>) {
