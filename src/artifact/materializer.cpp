...
MaterializedArtifact materialize(const Reader& reader, const MaterializationPlan& plan,
    ...
    // Mapped-host placements are copied through plain host memcpy rather than the overlapped
    // direct-I/O path above: they never move to VRAM, so there is no transfer stream to overlap
    // with and no alignment contract to satisfy.
    for (const HostMappedMaterialization& placement : plan.host_mapped_objects) {
        ...
        auto mapped = std::make_unique<MappedHostBuffer>(static_cast<std::size_t>(placement.bytes));

        std::memcpy(mapped->data(), payload.data.data(),
