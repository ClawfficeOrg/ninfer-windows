...
MappedHostBuffer::MappedHostBuffer(std::size_t size_bytes) {
    ...
    void* host              = nullptr;
    const cudaError_t alloc = cudaHostAlloc(&host, size_bytes, cudaHostAllocMapped);
    if (alloc != cudaSuccess) {
        throw std::runtime_error(cuda_error_message("cudaHostAllocMapped failed", alloc));
    }

    void* device             = nullptr;
    const cudaError_t mapped = cudaHostGetDevicePointer(&device, host, 0);
    if (mapped != cudaSuccess) {
        free_pinned(host);
        throw std::runtime_error(cuda_error_message("cudaHostGetDevicePointer failed", mapped));
    }
